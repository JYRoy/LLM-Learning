...
class GPT(nn.Module):
    ...
    def configure_optimizer(self, weight_decay, learning_rate, device):
        ...
        print(
            f"num nodecayed parameter tensors: {len(nodecay_params)}, with {num_nodecay_params:,} decay_params"
        ) 
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))  # logits.view(-1, logits.size(-1): (B*T, vocab_size), targets: (B*T)
        return logits, loss
